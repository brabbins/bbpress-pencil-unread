...
class bbP_Pencil_Unread_Bookmarks {
    ...
    function bookmark_embed_link($post_id = false){
        ...
        $bbppu_bookmark_url_attr = array();

        //setup hookable attributes for the url
				$bbppu_bookmark_url_attr = array(
					'html' 		=> '',
					'classes'	=> '',
				...
				// hook for url attributes: html, classes, icon, title
				// bbppu_bookmark_icon()
				// array $bbppu_bookmark_url_attr
				if ( has_filter( 'bbppu_bookmark_icon' ) ) {

					$bbppu_bookmark_url_attr = apply_filters( 'bbppu_bookmark_icon', $bbppu_bookmark_url_attr );

				}

				$url = $this->bookmark_get_url( $bookmark_id );

				printf( '<a %s class="%s bbppu-bookmark" title="%s" href="%s">%s</a>', wp_kses_post( $bbppu_bookmark_url_attr['html'] ), esc_attr( $bbppu_bookmark_url_attr['classes'] ), esc_attr( $bbppu_bookmark_url_attr['title'] ), esc_url( $url ), wp_kses_post( $bbppu_bookmark_url_attr['icon'] ) );
